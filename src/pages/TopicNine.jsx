  ...
  return (
    <div className="min-h-screen bg-[#FDFCFB] font-sans text-[#1A365D] overflow-x-hidden">
      ...
      <div className="max-w-xl mx-auto pt-24 pb-48 relative flex flex-col-reverse items-center">
        {/* De Slingerende Lijn (SVG) */}
        <svg className="absolute inset-0 w-full h-full -z-10 opacity-10" preserveAspectRatio="none">
          <path 
            d="M 250 2000 Q 400 1800 250 1600 T 250 1200 T 250 800 T 250 400 T 250 0" 
            fill="none" 
            stroke="#1A365D" 
            strokeWidth="4" 
            strokeDasharray="12"
            ...
            <div key={step.id} className="relative w-full flex justify-center py-14">
              ...
              >
                ...
                >
                  ...
                  <div className={`absolute -top-10 px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border
                    ${isLocked ? 'bg-white text-slate-300 border-slate-100' : 'bg-white text-[#1A365D] border-[#C5A059] shadow-lg'}`}>
                    Lvl {step.id}
                  </div>
                  ...
                  {isCurrent && (
                    <motion.div 
                      ...
                      className="absolute inset-0 rounded-[2.5rem] ring-[12px] ring-[#C5A059]/30 -z-10" 
                    />
                  )}
                </button>
              </motion.div>
...
// ==========================================
// DE SLIMME GAME-TRANSFORMER (FIXED)
// ==========================================
  ...
  const gameData = useMemo(() => {
    ...
    if (mode === 'TF') {
      ...
      if (isCorrectScenario) {
        displayOption = rawQ.options[rawQ.correct];
      } else {
        const distractors = rawQ.options.filter((_, i) => i !== rawQ.correct);
        displayOption = distractors[Math.floor(Math.random() * distractors.length)];
      }

      return {
        type: 'TF',
        question: rawQ.q,
        statement: displayOption,
        correctValue: isCorrectScenario,
        basis: rawQ.basis
      };
    }
    ...
    return {
      ...
      correctIdx: rawQ.correct,
      basis: rawQ.basis
    };
  }, [currentIdx, rawQ]); // Alleen herberekenen als de vraag verandert

  const handleAnswer = (val) => {
    if (selected !== null) return;
    setSelected(val);

    const isCorrect = gameData.type === 'MC' 
      ? val === gameData.correctIdx 
      : val === gameData.correctValue;
  ...
  return (
    <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="fixed inset-0 z-[100] bg-white flex flex-col">
      ...
      <div className="p-8 flex items-center gap-6">
        <button onClick={onLoss} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X size={32} className="text-slate-300"/></button>
        <div className="h-2 flex-1 bg-slate-100 rounded-full overflow-hidden">
          <motion.div className="h-full bg-[#C5A059]" animate={{ width: `${(currentIdx / questions.length) * 100}%` }} />
        </div>
      </div>

      <div className="flex-1 flex flex-col justify-center max-w-2xl mx-auto w-full px-8">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl md:text-5xl font-serif italic text-[#1A365D] leading-[1.2] px-4">
            {gameData.question}
          </h2>
        </div>
          ...
          {gameData.type === 'MC' ? (
            gameData.options.map((opt, i) => (
              <button
                ...
                className={`p-6 rounded-[2rem] border-2 text-left font-bold transition-all text-lg
            ...
            <div className="space-y-12">
              {/* De Screenshot Blauwe Kaart */}
              <motion.div 
                initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                className="p-12 bg-[#1A365D] rounded-[3.5rem] text-white text-center shadow-2xl relative border-t-[6px] border-[#C5A059]"
              >
                <p className="opacity-40 text-[9px] uppercase tracking-[0.3em] mb-6">Is deze stelling correct?</p>
                <p className="text-3xl font-serif italic leading-relaxed">"{gameData.statement}"</p>
              </motion.div>
              
              <div className="grid grid-cols-2 gap-6">
                <button onClick={() => handleAnswer(true)} className="py-8 bg-white border-2 border-slate-100 rounded-[2rem] font-black text-xs uppercase tracking-widest hover:bg-[#F8F9FA] transition-all active:scale-95 shadow-sm">Waar</button>
                <button onClick={() => handleAnswer(false)} className="py-8 bg-white border-2 border-slate-100 rounded-[2rem] font-black text-xs uppercase tracking-widest hover:bg-[#F8F9FA] transition-all active:scale-95 shadow-sm">Onwaar</button>
              </div>
            </div>
          )}
        </div>
      ...
      <AnimatePresence>
        {showError && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-[#ef4444] flex flex-col items-center justify-center text-white text-center p-12 z-[120]">
            <AlertCircle size={100} className="mb-8" />
            <h3 className="text-5xl font-serif italic mb-6 text-white">Foutief Oordeel</h3>
            <div className="bg-white/10 p-10 rounded-[3rem] backdrop-blur-xl border border-white/20 max-w-lg">
               <p className="text-xl leading-relaxed italic">"{gameData.basis}"</p>
            </div>
            <p className="mt-12 text-[10px] font-black uppercase tracking-[0.4em] animate-pulse">Je verliest een hartje...</p>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
